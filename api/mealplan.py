...
ingredients에는 장을 봐야 하는 재료만 적으세요. 소금, 후추, 설탕, 식용유, 참기름, 간장, 고춧가루,
물, 밥처럼 대부분의 가정에 이미 있는 기본 조미료와 주식은 목록에서 빼세요.
이 목록이 그대로 장보기 리스트가 되므로, 사러 갈 필요가 없는 항목이 섞이면 리스트가 쓸모없어집니다.
