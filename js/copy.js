...
const COPY = {
  ...
  PANTRY: {
    ...
    staplesTitle: "기본 재료",
    staplesCount: (count) => `${count}개 있음`,
    staplesHint: "실제로 있는 것만 넣어두면 추천할 때 정확히 반영해요.",
    staplesEmpty: "등록된 기본 재료가 없어요.",
    staplesAddLabel: "있는 기본 재료를 눌러서 추가하세요",
  ...
  UI: {
    ...
    /* "재료 한두 개 사서 해먹기"로 받은 결과. 카드마다 "사야 해요"가 붙는 게 당연한
       자리이므로 그건 놀랄 일이 아니라고 먼저 말해두고, 가장 적게 사도 되는 것이
       몇 개인지를 알려준다 — 서버가 적게 사는 순서로 올려 보낸다. */
    shoppingLead: (count) =>
      count === 1
        ? "하나만 사면 되는 것부터 보여드려요."
        : `가장 적게 사도 되는 게 ${count}개예요. 적게 사도 되는 순서로 보여드려요.`,

    /* 두 버튼은 서로의 막다른 길을 열어준다. 한쪽이 비면 다른 쪽을 권한다 —
       "재료를 다르게 입력해보세요"로 끝내면 사용자가 할 수 있는 게 없다. */
    emptyReadyMode: "지금 있는 재료로 딱 떨어지는 건 없어요. \"재료 한두 개 사서 해먹기\"를 눌러보세요.",
    emptyShoppingMode: "재료를 조금만 더 넣어주시면 사서 만들 수 있는 것까지 찾아드릴게요.",
    ...
    /* 추천 버튼 두 개. 한 버튼으로 물으면 "지금 되는 것"과 "사야 되는 것"이 한 목록에
       섞여 나와서, 오늘 저녁을 해결하러 온 사람이 카드를 하나씩 읽어 골라내야 했다.
       무엇을 원하는지는 사용자가 이미 알고 있으므로 묻는 단계에서 가른다.
       라벨에 "즉시"·"구매" 같은 말 대신 해먹는다는 말을 쓴다 — 이 서비스가 파는 것은
       검색이 아니라 오늘 한 끼다. */
    submitReady: "지금 있는 걸로 해먹기",
    submitShopping: "재료 한두 개 사서 해먹기",
