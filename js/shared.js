...
/* 냉장고 화면에서 일반 재료와 나눠 보여줄 기본 재료 이름. 예전처럼 모두 갖고 있다고
   가정하지 않는다 — 실제 보유 여부는 다른 재료와 똑같이 localStorage에 들어있는지로
   판단한다. 별칭도 같은 영역에 들어가게 함께 둔다.
   api/shopping.py의 PANTRY_STAPLES와 같은 목록이어야 한다. 그쪽은 이 이름들을 "냉장고에
   있으면 수량을 못 맞춰도 있는 것으로 본다"에 쓴다 — 한쪽만 늘리면 냉장고 화면은 기본
   재료 칸에 넣어두고 장보기는 매번 사라고 하는, 설명할 수 없는 상태가 된다. */
