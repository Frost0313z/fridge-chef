...
아래 화면은 모두 **배포된 사이트**(https://fridge-chef-gamma.vercel.app)에서 캡처한 것입니다.

**홈**

![홈 화면](./docs/screenshot-home-desktop.png)

**AI 기능 동작 예시**

냉장고에 `계란 · 대파 · 두부 · 김치 · 양파`를 넣어둔 상태에서 `두 끼 분량 / 15분 이내`로 요청한 결과입니다.

![추천 결과 화면](./docs/screenshot-recipe-desktop.png)
...
→ 추천 결과
김치 두부 계란 볶음  ⏱ 15분
재료: 계란 · 대파 · 두부 · 김치 · 양파   ← 5개 모두 냉장고에 있어 흐리게 표시됨
조리 순서: 1. 양파와 대파를 잘게 썬다 … 7. 소금과 후추로 간을 맞춘 후 그릇에 담아낸다
[만개의레시피에서 찾아보기 →]  (검색어: "두부볶음")
```

재료 이름이 모두 흐린 회색인 것은 다섯 가지가 전부 냉장고에 있다는 표시입니다 —
