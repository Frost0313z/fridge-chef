...
> - [🧠 설계 노트](#-설계-노트) — AI 응답 스키마 강제, 실패 처리 3종, **장보기 산수를 AI에서 코드로 옮긴 이유**, **재료 검증을 프롬프트가 아니라 코드로 강제한 이유**, 재료 이름 비교 규칙(+완제품 구분), **지금 바로 만들 수 있는 것을 맨 앞에 두는 이유**, 절약 체감을 주간/월간으로 나눈 이유, 빈 냉장고 처리, 반응형·접근성 전략
