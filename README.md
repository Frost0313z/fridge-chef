...
> - [🧠 설계 노트](#-설계-노트) — AI 응답 스키마 강제, 실패 처리 3종, 재료 이름 비교 규칙, 홈 진입 동선, 빈 냉장고 처리, 반응형·접근성 전략
> - [📄 서비스 기획서](./docs/서비스기획서.md)
> - [📐 UI/UX 원칙](./docs/UIUX원칙.md) — 안전(해를 끼치지 않는다) 7 · 배려(실제로 돕는다) 8 · 디자인(설명 없이 이해된다) 10, 그리고 완료 11건 / 남은 16건 체크리스트
> - [📄 라이선스](#-라이선스)
...
느려지는데, 서버 15초 · 클라이언트 20초(식단 25초) 이중 타임아웃이 걸려 있어 무한 대기 대신
...
식단은 `MAX_DAYS = 7`, 장보기는 `MAX_SHOPPING_ITEMS = 20`으로 잘라서 내보냅니다
(`api/recommend.py`, `api/mealplan.py`). 입력이 길어져도 화면이 끝없이 늘어나지는 않습니다.

> [!NOTE]
> 위 표는 **코드를 읽어 정리한 설계상 동작**입니다. 배포본에 실제로 35개 재료를 넣어 측정한
> 응답 시간·결과는 아직 이 문서에 기록하지 않았습니다.

**식단 계획의 30개 무시가 조용하다는 점은 알려진 한계입니다.** 31번째 재료부터는 사용자에게
아무 말 없이 빠지므로, 냉장고에 30개 넘게 등록한 사용자는 "분명 있는데 장보기 리스트에 또 떴다"를
겪을 수 있습니다. 안내 문구를 붙이는 것이 맞고, [UI/UX 원칙 체크리스트](./docs/UIUX원칙.md#d-체크리스트)에
남겨두었습니다.
...
| 식단 계획 (`mealplan.html`) | 냉장고 기반 며칠치 저녁 식단 AI 계획 |
| 장보기 (`shopping.html`) | 식단에 필요한 재료 중 냉장고에 없는 것만 추출(쿠팡 검색 링크) → "샀어요"로 냉장고에 반영 |

- **냉장고 단일 출처** — 재료 편집은 냉장고 페이지에서만. 나머지 화면은 헤더 아래 접이식 조회 바로 현황을 확인
- **장보기 → 냉장고 고리** — 산 재료는 "샀어요" 한 번으로 냉장고에 들어가고 리스트에서 사라짐
...
- **다른 요리 추천받기** — 결과가 마음에 안 들면 폼으로 돌아가지 않고 바로 재추천
...
- **[보너스]** 식단 계획 + 장보기 리스트 — 냉장고를 반영해 AI가 3/5/7일치 저녁 식단을 계획하고, 냉장고에 없는 재료만 걸러 쿠팡 검색 링크와 함께 보여줌 (장바구니 자동 담기는 하지 않음)
...
│   └── shopping.js         # 장보기 리스트 계산·렌더링, 샀어요 처리
├── api/
│   ├── recommend.py        # Vercel Serverless Function — 재료 기반 요리 추천
│   └── mealplan.py         # Vercel Serverless Function — 식단 계획
├── docs/
│   ├── 서비스기획서.md
