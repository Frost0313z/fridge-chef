...
| 레시피 추천 (`recipe.html`) | 재료/인원수/조리시간 입력 → AI 추천 요리 카드 표시. 재료마다 재료함 보유 여부를 함께 표시 |
| 주간 식단 (`mealplan.html`) | 재료함 기반 며칠치 식단 AI 계획 → 부족한 재료만 장보기 리스트(쿠팡 검색 링크 포함)로 추출 |
| 소개 (`about.html`) | 이용 방법 3단계, 기술 스택, FAQ |

- **추천 재료 중 무엇을 사야 하는지 표시** — 재료함과 대조해 이미 가진 재료는 흐리게, 없는 재료는 "사야 해요"로 구분 (재료함이 비어 있으면 표시하지 않음)
