...
- **[보너스]** 다크 모드 토글 — 선택값을 `localStorage`에 저장, 최초 방문 시 OS 설정(`prefers-color-scheme`)을 따름.
  버튼은 홈 화면 우측 상단에만 둡니다(상단 내비게이션에는 페이지 링크만 남깁니다)
...
- **응답 지연**: 서버 15초 + 클라이언트 20초로 이중 타임아웃을 걸어, 서버가 먼저 끊어도 클라이언트가 무한 대기하지 않도록 한다.
  ...
  막대가 끝에 닿는 순간이 실제로 `AbortController`가 끊는 순간이어서 화면과 동작이 어긋나지 않는다
...
| 경과 시간 표시 | 1초마다 바뀌는 숫자·막대는 `aria-live` 영역 **밖**에 두고 `aria-hidden` — 안에 두면 1초에 한 번씩 읽힌다. 대신 마감의 75%에서 "곧 알려드릴게요"를 `role="status"`로 한 번만 알림 |
