...
식단은 `MAX_DAYS = 7`일 x 3끼, 장보기는 `MAX_SHOPPING_ITEMS = 30`으로 잘라서 내보냅니다
...
| 냉장고 (`pantry.html`) | 재료를 넣고 빼고 한눈에 보기. 전체 목록을 보고 정리하는 화면 |
| 레시피 추천 (`recipe.html`) | 만들 분량/조리시간 입력 → AI 추천 요리 카드. 재료마다 냉장고 보유 여부를 함께 표시 |
| 식단 계획 (`mealplan.html`) | 냉장고 기반 며칠치 **아침·점심·저녁** AI 계획. "메뉴 바꾸기" 모드에서 고르고 놓아 끼니를 옮기거나 삭제 |
| 장보기 (`shopping.html`) | 식단에 필요한 재료 중 냉장고에 없는 것만 추출(쿠팡 검색 링크) → "샀어요"로 냉장고에 반영 |

- **어디서나 냉장고 편집** — 나머지 화면도 헤더 아래 접이식 바에서 재료를 추가·삭제할 수 있다. 흐름을 끊고 냉장고로 넘어가면 적던 재료와 결과가 날아가기 때문. 삭제에는 **되돌리기**가 붙는다 — 확인창은 맞게 지우는 99번을 방해하고, 되돌리기는 틀린 1번만 구제한다
...
- **[보너스]** 식단 계획 + 장보기 리스트 — 냉장고를 반영해 AI가 3/5/7일치 아침·점심·저녁 식단을 계획하고(계획 후 메뉴를 다른 끼니로 옮기거나 지울 수 있음), 식단에 필요한 양에서 냉장고에 있는 만큼을 뺀 나머지를 쿠팡 검색 링크와 함께 보여줌 (장바구니 자동 담기는 하지 않음)
...
│   ├── mealplan.py         # 식단 계획 + 장보기 목록 (handle 함수만 제공)
